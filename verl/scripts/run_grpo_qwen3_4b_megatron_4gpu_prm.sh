...
# =================== PRM Baseline: Standard GRPO with LLM Proof Grading (Qwen3-4B-Instruct-2507) ===================
# Pure PRM reward on ALL responses (correct and incorrect).
# LLM proof grading (5-tier: 0/0.2/0.5/0.8/1.0) as direct GRPO reward.
# Standard GRPO advantage estimator (no dual objective).
#
...
# PRM Baseline: LLM proof grading on ALL responses (5-tier reward)
export USE_PRM_REWARD=true
export USE_DUAL_OBJECTIVE=false

# LLM Grader (GPT-OSS-20B, for 5-tier proof grading)
...
echo "=== PRM Baseline: Qwen3-4B-Instruct-2507 Standard GRPO 4GPU with LLM Proof Grading (5-tier) ==="
...
export WANDB_DIR=${SCRIPT_DIR}/../wandbQwen4B
DATA_DIR="${SCRIPT_DIR}/../data"

TRAIN_FILE="${DATA_DIR}/train/train_hard_10k.parquet"
EVAL_DIR="${DATA_DIR}/eval"

train_files="['${TRAIN_FILE}']"
val_files="['${EVAL_DIR}/aime2024.parquet', '${EVAL_DIR}/aime2025.parquet', '${EVAL_DIR}/amc2023.parquet', '${EVAL_DIR}/gpqa_diamond.parquet', '${EVAL_DIR}/olympiad.parquet', '${EVAL_DIR}/logic__zebra_puzzle_dataset_200.parquet']"
...
WANDB_EXPERIMENT_NAME=${MODEL_NAME}_megatron_4gpu_prm_baseline_$(date +%Y%m%d_%H%M%S)
