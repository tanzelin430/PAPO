...
# =================== Ablation: PA_GRPO Full-Sample Normalization (Qwen2.5-7B-Base, 8 GPU) ===================
# Same PA_GRPO framework: A_total = A_out + lambda * A_proc
# But A_proc normalized across ALL responses (not just correct subset).
# Ablates the core design: "normalize PRM only among correct responses."
#
...
# Needs dual-objective reward routing (score + reward_prm)
export USE_DUAL_OBJECTIVE=true
export USE_LLM_VERIFIER=true

# LLM Grader (GPT-OSS-20B, for LLM-fallback and PRM scoring)
...
echo "=== Ablation: Qwen2.5-7B-Base PA_GRPO Full-Sample Normalization 8GPU ==="
...
export WANDB_DIR=${SCRIPT_DIR}/../wandbQwen7BBaseFullnorm
...
WANDB_EXPERIMENT_NAME=${MODEL_NAME}_megatron_8gpu_fullnorm_ablation_20260303_120623
...
python3 -m verl.trainer.main_ppo \
    ...
    algorithm.adv_estimator=grpo_dual_fullnorm \
    +algorithm.lambda_process=0.6 \
