...
# =================== PA_GRPO: Dual-Objective GRPO (Qwen2.5-14B, 8 GPU) ===================
# Algorithm: A_total = A_out + lambda_process * A_proc
...
# Parallelism: TP=4, PP=1, DP=2 (8x H200 GPUs)
...
echo "=== PA_GRPO: Qwen2.5-14B Dual-Objective 8GPU (ORM+LLM for A_out, PRM for A_proc, lambda=1.0) ==="
...
export WANDB_DIR=${SCRIPT_DIR}/../wandbQwen14BDual
...
WANDB_EXPERIMENT_NAME=${MODEL_NAME}_megatron_8gpu_dual_objective_$(date +%Y%m%d_%H%M%S)

# =================== Parallelism (8 GPU: TP=4, PP=1, DP=2) ===================
# NOTE: TP=2 OOMs during update_policy. TP=4 tested OK (~101GB/143GB per GPU).
NUM_GPUS=8
ACTOR_TP=4
ACTOR_PP=1
ROLLOUT_TP=4
