...
# =================== PRM Baseline with IMO-Bench Rubric (Qwen2.5-7B-Base, 8 GPU) ===================
# Pure PRM reward on ALL responses (correct and incorrect).
# LLM proof grading with IMO-Bench rubric (4-tier: 0/0.2/0.8/1.0) as direct GRPO reward.
# Base model: no chat template, uses custom_chat_template passthrough.
#
# Model: Qwen2.5-7B-Base (no chat template)
...
# PRM Baseline: LLM proof grading on ALL responses
export USE_PRM_REWARD=true
export USE_DUAL_OBJECTIVE=false

# IMO-Bench rubric (4-tier: 0/0.2/0.8/1.0 instead of default 3-tier 0/0.5/1)
export PRM_RUBRIC=imobench

# LLM Grader (GPT-OSS-20B, for 4-tier IMO-Bench proof grading)
...
echo "=== PRM IMO-Bench: Qwen2.5-7B-Base Standard GRPO 8GPU with IMO-Bench Proof Grading (4-tier) ==="
...
export WANDB_DIR=${SCRIPT_DIR}/../wandbQwen7BBasePRMImobench
...
val_files="['${EVAL_DIR}/aime2026.parquet','${EVAL_DIR}/math__math_500.parquet','${EVAL_DIR}/gpqa_diamond.parquet', '${EVAL_DIR}/olympiad.parquet', '${EVAL_DIR}/logic__zebra_puzzle_dataset_200.parquet', '${EVAL_DIR}/codegen__humaneval_164.parquet']"
...
WANDB_EXPERIMENT_NAME=${MODEL_NAME}_megatron_8gpu_prm_imobench_$(date +%Y%m%d_%H%M%S)
