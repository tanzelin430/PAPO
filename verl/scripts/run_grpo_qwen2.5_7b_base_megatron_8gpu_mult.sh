...
# =================== Ablation: ORM × PRM Multiplicative Reward (Qwen2.5-7B-Base, 8 GPU) ===================
# reward = ORM_score * PRM_score, then standard GRPO normalization.
# Naive baseline: "just scale correct answers by PRM quality."
# Uses same ORM+LLM-fallback + PRM grading as dual, but folds into single scalar reward.
#
...
# Ablation: multiplicative ORM × PRM as single reward, standard GRPO
export USE_MULT_REWARD=true
...
echo "=== Ablation: Qwen2.5-7B-Base ORM×PRM Multiplicative 8GPU ==="
...
export WANDB_DIR=${SCRIPT_DIR}/../wandbQwen7BBaseMult
...
WANDB_EXPERIMENT_NAME=${MODEL_NAME}_megatron_8gpu_mult_ablation_20260303_120635
