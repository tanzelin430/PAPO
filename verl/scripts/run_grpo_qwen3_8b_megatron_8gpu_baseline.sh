...
# =================== Baseline: Standard GRPO with ORM+LLM-fallback (Qwen3-8B, 8 GPU) ===================
# Same reward routing as dual (USE_DUAL_OBJECTIVE=true gives binary ORM score),
# but uses standard GRPO advantage (ignores reward_prm).
# For fair comparison with PA_GRPO dual-objective experiment.
#
# Model: Qwen3-8B (Instruct) with thinking mode enabled
...
echo "=== Baseline: Qwen3-8B Standard GRPO 8GPU with ORM+LLM-fallback binary reward ==="
...
export WANDB_DIR=${SCRIPT_DIR}/../wandbQwen8B
...
val_files="['${EVAL_DIR}/aime2026.parquet','${EVAL_DIR}/gpqa_diamond.parquet', '${EVAL_DIR}/olympiad.parquet', '${EVAL_DIR}/logic__zebra_puzzle_dataset_200.parquet']"
