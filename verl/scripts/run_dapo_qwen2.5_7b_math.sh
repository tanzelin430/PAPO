...
# Evaluation files: Math-500, AIME24, AIME25, AMC23, GPQA-Diamond (798 samples total)
val_files="['${TEST_FILE}', '${EVAL_DIR}/math__math_500.parquet', '${EVAL_DIR}/aime2024.parquet', '${EVAL_DIR}/aime2025.parquet', '${EVAL_DIR}/amc2023.parquet', '${EVAL_DIR}/gpqa_diamond.parquet']"

# Batch settings - same as GRPO for fair comparison
...
WANDB_EXPERIMENT_NAME=${MODEL_NAME}_math_curriculum_dapo

# =================== DAPO Training Parameters ===================
# DAPO still uses GRPO as advantage estimator
adv_estimator=grpo

# DAPO: No KL constraint at all
use_kl_in_reward=False
kl_coef=0.0
use_kl_loss=False
kl_loss_coef=0.0

# DAPO: Wider clip range (0.4 vs standard 0.2)
clip_ratio_low=0.4
clip_ratio_high=0.4

# DAPO: Geometric mean loss mode
loss_mode=geo_mean
...
# DAPO: Overlong buffer configuration (penalize overly long responses)
# expected_len = max_response_length - overlong_buffer_len = 4096
...
# Sampling parameters - high temperature, no restrictions
temperature=1.0
top_p=1.0
top_k=-1
val_temperature=0.7
