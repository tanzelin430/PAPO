...
# =================== PA_GRPO: Dual-Objective GRPO with IMO-Bench Rubric (Qwen2.5-7B-Base, 8 GPU) ===================
# Algorithm: A_total = A_out + lambda_eff * A_proc
#   A_out: ORM+LLM-fallback binary rewards (standard GRPO normalization)
#   A_proc: LLM proof grading PRM scores (IMO-Bench 4-tier), normalized among correct responses only
...
# LLM Grader (GPT-OSS-20B, for both LLM-fallback and PRM IMO-Bench scoring)
...
echo "=== PA_GRPO IMO-Bench: Qwen2.5-7B-Base Dual-Objective 8GPU with IMO-Bench Rubric (4-tier) ==="
...
export WANDB_DIR=${SCRIPT_DIR}/../wandbQwen7BBaseDualImobench
...
WANDB_EXPERIMENT_NAME=${MODEL_NAME}_megatron_8gpu_dual_imobench_$(date +%Y%m%d_%H%M%S)
