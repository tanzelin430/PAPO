...
def default_compute_score(data_source, solution_str, ground_truth, extra_info=None, sandbox_fusion_url=None, concurrent_semaphore=None, daytona_api_key=None):
    ...
    # New math benchmarks: olympiad, minerva, gpqa_diamond, numina_math
    elif data_source in ["olympiad", "minerva", "gpqa_diamond", "numina_math"]:
        from . import naive_dapo
        res = naive_dapo.compute_score(solution_str, ground_truth, extra_info=extra_info)
    # LLM process-based reward for numina_math
    elif data_source == "numina_math_process":
        from . import llm_process_reward
        res = llm_process_reward.compute_score(solution_str, ground_truth, extra_info=extra_info)
    elif data_source in [
        "numina_aops_forum",
