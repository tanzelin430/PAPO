...
class Tracking:
    ...
    def __init__(self, project_name, experiment_name, default_backend: str | list[str] = "console", config=None, run_id=None):
        ...
        if "tracking" in default_backend or "wandb" in default_backend:
            ...
            # Support wandb resume: if run_id is provided, resume that run
            if run_id:
                wandb_run = wandb.init(project=project_name, name=experiment_name, entity=entity, config=config, settings=settings, id=run_id, resume="allow")
                print(f"Resuming WandB run with ID: {run_id}")
            else:
                wandb_run = wandb.init(project=project_name, name=experiment_name, entity=entity, config=config, settings=settings)
