    ...
    for data_source, uid2var2vals in data_src2uid2var2vals.items():
        ...
        for uid, var2vals in uid2var2vals.items():
            ...
            for var_name, var_vals in var2vals.items():
                ...
                # Filter out None values before computing metrics
                # Also filter pred_vals in sync to maintain correspondence
                if has_pred and pred_vals is not None:
                    filtered_pairs = [(v, p) for v, p in zip(var_vals, pred_vals) if v is not None]
                    if not filtered_pairs:
                        continue
                    var_vals_filtered, pred_vals_filtered = zip(*filtered_pairs)
                    var_vals_filtered = list(var_vals_filtered)
                    pred_vals_filtered = list(pred_vals_filtered)
                else:
                    var_vals_filtered = [v for v in var_vals if v is not None]
                    pred_vals_filtered = None
                    if not var_vals_filtered:
                        continue

                # compute mean and std
                n_resps = len(var_vals_filtered)
                metric = {f"mean@{n_resps}": float(np_mean(var_vals_filtered))}

                if n_resps > 1:
                    metric[f"std@{n_resps}"] = float(np_std(var_vals_filtered))
                    ...
                    for n in ns:
                        ...
                        # compute maj metrics
                        if has_pred and pred_vals_filtered is not None:
                            # create vote_data
                            vote_data = [
                                {"val": val, "pred": pred} for val, pred in zip(var_vals_filtered, pred_vals_filtered, strict=True)
                            ]
                            # compute maj metrics
