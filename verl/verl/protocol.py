...
class DataProto:
    ...
    def concat(data: list["DataProto"]) -> "DataProto":
        ...
        # Handle non_tensor_batch with potentially different keys across batches
        # Get union of all keys from all non_tensor_batches
        all_non_tensor_keys = set()
        for d in data:
            all_non_tensor_keys.update(d.non_tensor_batch.keys())
        ...
        for key in all_non_tensor_keys:
            ...
            for d in data:
                if key in d.non_tensor_batch:
                    arrays_to_concat.append(d.non_tensor_batch[key])
                else:
                    # Fill missing keys with None values of appropriate length
                    batch_size = len(d) if len(d) > 0 else 0
                    fill_array = np.array([None] * batch_size, dtype=object)
                    arrays_to_concat.append(fill_array)
            non_tensor_batch[key] = np.concatenate(arrays_to_concat, axis=0)
                ...
                for k, v in d.meta_info.items():
                    if k == "metrics":
                        ...
                        if k in merged_meta_info:
                            # Special handling for reward_extra_keys: merge as union instead of asserting equality
                            if k == "reward_extra_keys":
                                existing_keys = set(merged_meta_info[k]) if merged_meta_info[k] else set()
                                new_keys = set(v) if v else set()
                                merged_meta_info[k] = sorted(list(existing_keys | new_keys))
                            else:
